...
STORAGE_DIR = Path("storage/biometrics")
...
async def upload_biometric(
    ...
    if file.content_type not in ALLOWED_CONTENT_TYPES:
        raise HTTPException(
            ...
            detail="Unsupported file type for biometric image. Allowed: JPEG, PNG",
        )
    ...
    biometric_id = f"BIO-{uuid4().hex[:8].upper()}"
    extension = ALLOWED_CONTENT_TYPES[file.content_type]
    filename = f"{biometric_id}{extension}"
    ...
    cloud_result = upload_to_cloudinary(
        ...
        folder="trinetra/biometrics",
        public_id=biometric_id,
        resource_type="image",
    )
    ...
    await database.biometric_results.insert_one(biometric_record)

    await database.verification_sessions.update_one(
        {"session_id": session_id},
        {
            "$set": {
                "biometric_result_id": biometric_id,
                "status": "BIOMETRIC_UPLOADED",
                "updated_at": now,
            }
        },
    )
